...
<div class="container" style="margin-top:3%; margin-bottom:5%">
    <div class="row">
        ...
        @if ($doc->mahasiswa_id == $calonmahasiswa->id)
        @if (is_null($doc-> status_rekomendasi))
        <a class="btn btn-success" style="border-radius: 40px; width:20%;margin-right:1%"
            href="{{url('/terima',$calonmahasiswa->id)}}"> Terima Verifikasi </a>
        <a class="btn btn-danger" style="border-radius: 40px; width:20%;"
            href="{{url('/tolak',$calonmahasiswa->id)}}">Tolak Verifikasi</a>
        @elseif ($doc-> status_rekomendasi == 1)
        <a class="btn btn-secondary" style="border-radius: 40px; width:20%;margin-right:1%"
            href="{{url('/ralat',$calonmahasiswa->id)}}"> Ralat Verifikasi </a>
        <a class="btn btn-danger" style="border-radius: 40px; width:20%;"
            href="{{url('/tolak',$calonmahasiswa->id)}}">Tolak Verifikasi</a>
        @else
        <a class="btn btn-success" style="border-radius: 40px; width:20%;margin-right:1%"
            href="{{url('/terima',$calonmahasiswa->id)}}"> Terima Verifikasi</a>
        <a class="btn btn-secondary" style="border-radius: 40px; width:20%;"
            href="{{url('/ralat',$calonmahasiswa->id)}}">Ralat Verifikasi</a>
        @endif
        @endif
        @endforeach -->
